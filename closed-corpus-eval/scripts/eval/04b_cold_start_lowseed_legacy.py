...
(LEGACY — superseded 2026-07-14 by 04b_cold_start_lowseed.py)

**No longer the canonical experiment.** Archived here (not deleted) because
its Pareto hub filter has a real, documented flaw: it discards newly-found
CANDIDATE papers based on the candidate's own out-degree, so a true gold
paper can be excluded purely for citing a lot of things itself — a property
unrelated to its relevance. The current canonical 04b_cold_start_lowseed.py
uses litdiscover's real production operators (backward_traversal_operator /
forward_traversal_operator / pareto_hub_threshold via a ClosedCorpusSource),
whose filter only decides whether to expand an already-visited frontier
paper's citers, based on that frontier paper's own citation count — it never
discards a candidate based on the candidate's own properties.

Full side-by-side comparison (54 conditions: 3 surveys x 3 seed strategies x
k in {1,2,3,4,5,10}) run 2026-07-14: mean recall 93.5% (this script) vs
99.6% (production-aligned), mean corpus size 205,021 (this script) vs 66,023
(production-aligned, 3.1x smaller) — see
wiki/litdiscover/phase-discovery-roadmap.md §1.3 for the full account,
including the one legible trade-off (contaminated-seed, low-k conditions
saw small recall decreases, 1.9-4.9pp, from less indiscriminate exploration
within the fixed 2-round budget).

Results from this legacy run are preserved at
data/outputs/cold_start_results_lowseed_legacy.json and
data/outputs/figures_lowseed_legacy/ for reference. This script's own
original docstring, describing the "no Pareto filter... production concern
for heterogeneous open-web corpora" reasoning, is left below unchanged as a
record of what the original design rationale was — see the note above for
why that rationale, evaluated empirically, didn't hold up.
