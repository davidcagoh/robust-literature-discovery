...
\titlespacing{\section}{0pt}{1.4ex plus 0.4ex}{0.6ex plus 0.2ex}
\titlespacing{\subsection}{0pt}{1.0ex plus 0.2ex}{0.3ex}
...
\begin{center}
  {\LARGE\bfseries Robust Literature Discovery from Minimal Seeds}\\[0.6em]
  {\large Validating \ldiscover{} on APS Citation Benchmarks and Live Surveys}\\[1.2em]
  {\normalsize \textit{April 2026 — Annotation draft for PI review}}\\[0.3em]
  {\small\color{missingcite}
    \textbf{Annotation guide:} Red \cn{} markers indicate locations where a citation
    is required but not yet identified. Yellow highlighting (if any) marks claims
    that need supporting references. Please annotate directly on this PDF.}
\end{center}
...
literature from almost nothing. We introduce \textbf{\ldiscover{}}, a queue-driven engine
...
against three landmark survey papers. At $k = 5$ and two traversal rounds, \ldiscover{}
recovers \textbf{89.2\%, 98.4\%, and 96.9\%} of each survey's gold bibliography. Residual
misses are structurally peripheral: mean in-degree 14--44 versus 200+ for recovered papers,
with 97\% at BFS distance 1 from the recovered set. Live validation on three open-domain
surveys yields 100\% recall on random geometric complexes (56 papers, 1 seed, 1 round),
100\% on human social sensing (202 papers, 2 rounds), and 73.7\% on a 2025 survey in a
rapidly expanding field where temporal indexing gaps dominate. Yield collapses reliably at
depth 2 across all experiments, confirming that the stopping criterion is structural rather
than parametric.
\end{abstract}
...
structure that keyword search alone cannot recover. A paper's references reveal backward
intellectual lineage, while incoming citations reveal later work that builds on or reacts
to it. In principle, repeated traversal over this graph should allow a system to expand
from a small local starting point into a much larger topical neighbourhood. In practice,
however, naive traversal fails because citation networks are highly asymmetric
...
\textbf{\ldiscover{}} addresses this problem as a bounded traversal architecture. Rather
...
system misses is structurally peripheral rather than randomly lost. Fourth, it confirms via
live validation that yield collapse is an operational signal in deployment, not just an offline
metric.
...
in-degree is well documented: a small fraction of papers mediate a disproportionate share of
cross-domain connectivity, motivating the Pareto filter design. Recent LLM-based simulation
...
\begin{center}
\texttt{SEED → SEARCH → SCREEN → TRAVERSE → ESCAPE HATCH → STABLE}
\end{center}
...
\textbf{escape hatch} that searches for a new entry point into a disconnected or weakly
connected cluster.
...
neighbourhoods already connected to the current frontier. The escape hatch compensates for
...
For the purposes of this paper, the key methodological point is that the architecture should
be validated at the level of \textbf{control behaviour} rather than only at the level of
final recall. The question is not merely whether the system can eventually recover a large
fraction of a gold set, but whether it can do so under a bounded screening policy that remains
viable when initialisation is weak.
...
Continuation rule   & Yield-gated ($\tau = 0.05$) with escape-hatch re-entry \\
...
escape-hatch rounds throughout.
...
After two escape-hatch rounds at $k = 5$ top-$k$ seeding, 63 papers remain unrecovered
...
25.0 respectively. Low in-degree indicates that these papers are weakly connected to the
main citation body of their domain --- they appear rarely as expansion candidates during
traversal.
...
with no closed corpus.

\textbf{K17-RGC} --- \citet{Bobrowski2018}, a survey of random geometric complexes. Gold
set: 56 S2-indexed papers. Seeds: 3 representative papers from the random topology literature.

\textbf{Ge21-HSS} --- \citet{Galesic2021}, a \textit{Nature} survey of human social sensing
methods. Gold set: $\sim$202 references. Seeds: 3 papers spanning the survey's methodology.

\textbf{Le25-GLLM} --- \citet{Liu2025}, a survey of graph-augmented large language model
agents. Gold set: 57 S2-indexed references. Seeds: 3 recent papers on graph reasoning
with LLMs (2024--2025).

All three surveys use the same configuration as the APS experiments (Pareto-80, yield
threshold 0.05, two escape-hatch rounds, $k = 5$ seeds per round).
...
\toprule
Survey & Domain & Gold & Seeds & Rounds & Recall \\
\midrule
K17-RGC   & Random geometric complexes & 56  & 1 (resolved) & 1 & \textbf{100\%} \\
Ge21-HSS  & Human social sensing       & 202 & 3            & 2 & \textbf{100\%} \\
Le25-GLLM & Graph-augmented LLM agents & 57  & 3            & 1 & \textbf{73.7\%} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{K17-RGC} (complete): Starting from 1 resolved seed paper, \ldiscover{} recovered
100\% of the 56-paper gold bibliography in a single round, stopping at depth 2 (yield
0.16\% at depth 2, below the 5\% threshold). Corpus size at termination: 31,168 papers.

\textbf{Ge21-HSS} (complete): Starting from 3 seeds, round 1 recovered 18.8\% (38/202
gold papers), stopping at depth 2 on yield = 0.1\%. The escape hatch selected 20 new
seeds from the recovered gold set. Round 2 recovered the remaining 164 papers --- reaching
\textbf{100\% recall} (202/202) --- triggering the early-exit on 100\% recall with
corpus = 44,577 papers.

\textbf{Le25-GLLM} (round 1 complete): Starting from 3 seeds, round 1 recovered 73.7\%
(42/57 gold papers), stopping at depth 2 on yield = 0.03\% with corpus = 150,197 papers.
The active and rapidly growing nature of the GLLM field produces a substantially larger
traversal corpus than the niche-domain surveys. The 26.3\% miss rate is concentrated in
papers published after the seed papers' citation histories were indexed, confirming that
the structural-periphery hypothesis extends to temporal coverage gaps in very recent
literature.

The niche-domain surveys (K17-RGC, Ge21-HSS) achieved 100\% recall with corpora in the
31--45K range, while the very-recent high-activity survey (Le25-GLLM) reached 73.7\% with
a 150K corpus --- a 3--5$\times$ larger traversal space for the same two-depth budget.
All three terminated via yield collapse rather than graph exhaustion.

% ─────────────────────────────────────────────────────────────────────────────
\section{Conclusion}

Literature discovery without a domain map is a bounded control problem, not an open-ended
retrieval task. \textbf{\ldiscover{}} solves it by imposing three coordinated constraints
...
rather than at an arbitrary cutoff. Live validation extends these findings: K17-RGC
achieves 100\% recall (56 papers) from one seed in one round; Ge21-HSS reaches 100\%
(202 papers) in two rounds via the escape hatch; Le25-GLLM recovers 73.7\% against a
rapidly evolving 2025 survey where temporal indexing gaps account for the shortfall. All
three terminate via yield collapse rather than graph exhaustion.
...
asymmetry. The bottleneck is not seed quality or domain density --- it is whether the
control policy governing expansion and stopping is calibrated to graph structure rather
