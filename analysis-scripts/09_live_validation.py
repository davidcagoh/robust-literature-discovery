...
Design principles:
  - All S2 responses are cached to disk (data-live/cache/). Re-runs are free.
  - Gold sets are extracted once and saved to data-live/gold-sets/; manual corrections
...
Usage:
  ...
  python3 analysis-scripts/09_live_validation.py --survey Ge21-HSS
"""
...
def build_gold_set(survey_id: str, pdf_path: Path,
    ...
    Saves to data-live/gold-sets/{survey_id}_gold.json.
...
def resolve_seeds(survey_id: str, seed_pdfs: list[Path]) -> list[dict]:
    ...
    Saves to data-live/seeds/{survey_id}_seeds.json.
    """
