%% litdiscover_tois.tex — ACM TOIS anonymous submission (acmart, manuscript mode)
%% Compile: pdflatex litdiscover_tois && bibtex litdiscover_tois && pdflatex litdiscover_tois && pdflatex litdiscover_tois
%%
%% ACCEPTANCE CHECKLIST:
%%   1. Remove 'anonymous,review' from \documentclass options
%%   2. Uncomment the real \author / \affiliation / \email block below
%%   3. Fill \acmYear, \acmMonth, \acmVolume, \acmNumber, \acmArticle, \acmDOI per production instructions
%%   4. Set appropriate \setcopyright{}
%%   5. Verify CCS concept IDs at https://dl.acm.org/ccs

\documentclass[manuscript,review,anonymous]{acmart}

%% acmart pre-loads: graphicx, hyperref, url, natbib, xcolor, inputenc, fontenc
%% Only add packages NOT already loaded by acmart:
...
%% ── ACM copyright (none for anonymous review) ────────────────────────────────
\setcopyright{none}
\copyrightyear{2026}

%% ── Journal block ─────────────────────────────────────────────────────────────
\acmJournal{TOIS}
%% \acmVolume{}  %% fill in after acceptance
%% \acmNumber{}  %% fill in after acceptance
%% \acmArticle{} %% fill in after acceptance
%% \acmYear{2026}
%% \acmMonth{}   %% fill in after acceptance
%% \acmDOI{}     %% fill in after acceptance
...
%% ── Figure path (relative from tois-submission/ → pub_figures/) ─────────────
\graphicspath{{../../closed-corpus-eval/data/outputs/pub_figures/}}
...
%% Authors are suppressed by the [anonymous] class option (ACM double-anonymous review).
%% For camera-ready: remove [anonymous] from \documentclass and uncomment below.
%% Solo-authored — no co-author on this paper.
%%
%% \author{David Goh}
%% \orcid{0009-0009-7241-6906}
%% \affiliation{%
...
\begin{document}
% acmart requires \begin{abstract} BEFORE \maketitle
\begin{abstract}
...
\begin{figure*}[htbp]
  ...
  \Description{Log-log plots of in-degree and out-degree distributions for the APS citation
  graph. In-degree follows a heavy-tailed power law; out-degree has a narrow, bounded range.}
  \label{fig:degree}
\end{figure*}
...
\begin{figure*}[htbp]
  ...
  \Description{Line chart showing BFS overlap with gold bibliography across depths 0--3,
  comparing backward-only, forward-only, bidirectional, and Pareto-80-filtered bidirectional
  traversal strategies for three APS surveys.}
  \label{fig:bfs}
\end{figure*}
...
\begin{figure*}[htbp]
  ...
  reduced corpus size.}
  \Description{Scatter plot of recall against corpus size at BFS depth 3, comparing
  backward, forward, bidirectional, and Pareto-filtered strategies across three surveys.
  Pareto-filtered bidirectional achieves near-maximum recall with the smallest corpus.}
  \label{fig:strategy}
\end{figure*}
...
\begin{figure*}[htbp]
  ...
  \Description{Stacked bar charts showing the number of gold papers recovered at each BFS
  depth and round for three APS surveys. Recovery drops sharply after depth 1--2 in all cases.}
  \label{fig:yield}
\end{figure*}
...
\begin{figure*}[htbp]
  ...
  round 2.}
  \Description{Bar charts showing recall after round 1 and round 2 for top-k, random, and
  contaminated seeds at k=5, across three APS surveys S1, S2, and S3.}
  \label{fig:rounds}
\end{figure*}
...
\begin{figure*}[htbp]
  ...
  \Description{Line charts showing final recall after two rounds as a function of seed size k
  from 1 to 10, with separate lines for top-k, random, and contaminated seed types across
  three surveys.}
  \label{fig:seedsize}
\end{figure*}
...
\begin{figure*}[htbp]
  ...
  \Description{Two panels: left shows overlapping in-degree histograms (log scale) for
  missed and recovered papers across S1, S2, S3, with missed papers concentrated at low
  in-degree. Right shows bar charts of BFS distance from recovered set to missed papers,
  with the majority at distance 1.}
  \label{fig:miss}
\end{figure*}
...
Generative AI tools were used as a coding assistant during development of the analysis
pipeline (ground-truth extraction, traversal simulation, cold-start experiments, and figure
generation) and as a writing and editing aid during manuscript preparation. All methodological
decisions, parameter choices, and interpretation of results were made and verified by the
author(s), who take full responsibility for the accuracy and integrity of the reported work.
