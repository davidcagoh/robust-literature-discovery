%% litdiscover_ipm_anonymous.tex — "Manuscript without author details" upload for IP&M's
%% anonymized review. Derived from litdiscover_ipm.tex with all author-identifying info
%% stripped (frontmatter author/affiliation/email block + CRediT statement name).
%% If the paper itself changes, edit litdiscover_ipm.tex first and re-derive this file.
%% Compile: pdflatex litdiscover_ipm_anonymous && bibtex litdiscover_ipm_anonymous && pdflatex litdiscover_ipm_anonymous && pdflatex litdiscover_ipm_anonymous
...
%% acmart's \Description{} accessibility macro has no equivalent requirement here;
%% keep the calls in figure captions as a no-op so the alt-text is preserved but inert.
\newcommand{\Description}[1]{}

%% ── Figure path (relative from ipm-submission/ → pub_figures/) ──────────────
...
\begin{keyword}
literature discovery \sep citation graph traversal \sep systematic review \sep snowballing \sep bibliographic retrieval
\end{keyword}
...
\textbf{Author:} Conceptualization, Methodology, Software, Validation, Formal analysis,
...
author, who takes full responsibility for the accuracy and integrity of the reported work.
